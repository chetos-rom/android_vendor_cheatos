...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the CheatOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch cheatos_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/cheatos-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.cheatos.device | grep -q "$LINEAGE_BUILD"); then
...
function repopick() {
    T=$(gettop)
    $T/vendor/cheatos/build/tools/repopick.py $@
}
